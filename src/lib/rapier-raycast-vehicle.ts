    ...
    private updateFriction(delta: number): void {
        ...
        for (let i = 0; i < this.wheels.length; i++) {
            ...
            if (wheel.state.inContactWithGround && wheel.state.groundRigidBody) {
                ...
                forwardWS.crossVectors(axle, surfNormalWS)
                forwardWS.normalize()
...
function resolveSingleBilateralConstraint(
    ...
    // At low speed: use full denominator (includes rotational inertia)
    // to prevent yaw feedback loop when idle.
    // At driving speed: use translational-only denominator
    // to preserve lateral grip during turns.
    const invMassDenom = body1.invMass() + body2.invMass()
    const fullDenom =
        computeImpulseDenominator(body1, pos1, normal) +
        computeImpulseDenominator(body2, pos2, normal)

    const lowSpeedImpulse = fullDenom > 0 ? (-contactDamping * rel_vel) / fullDenom : 0
    const highSpeedImpulse = invMassDenom > 0 ? (-contactDamping * rel_vel) / invMassDenom : 0

    const bodyVel = body1.linvel() as Vector3
    const speed = Math.sqrt(bodyVel.x * bodyVel.x + bodyVel.z * bodyVel.z)
    const blend = Math.min(1, speed / 2)

    return highSpeedImpulse * blend + lowSpeedImpulse * (1 - blend)
}
